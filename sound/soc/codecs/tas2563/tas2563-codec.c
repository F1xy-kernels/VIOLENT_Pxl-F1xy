...
static int fw_parse(struct tas2563_priv *pTAS2563, struct TFirmware *pFirmware,
...
static bool tas2563_get_coefficient_in_block(struct tas2563_priv *pTAS2563,
					     ...
					     int *pnValue);
int tas2563_set_program(struct tas2563_priv *pTAS2563, unsigned int nProgram,
			int nConfig);
		  ...
		  unsigned char len)
{
	...
	if (nResult == 0)
		nResult = isInBlockYRAM(pTAS2563, pCRCData, nBook, nPage, nReg,
					len);
			       ...
			       unsigned char nReg, unsigned char nValue)
{
	...
	if (nResult == 1) {
		nResult = pTAS2563->read(
			pTAS2563, TAS2563_REG(nBook, nPage, nReg), &nData1);
		if (nResult < 0)
			goto end;
			      ...
			      struct TBlock *pBlock)
{
	...
	dev_info(pTAS2563->dev,
		 "TAS2563 load block: Type = %d, commands = %d\n",
...
start:
	...
	while (nCommand < pBlock->mnCommands) {
		...
		if (nOffset <= 0x7F) {
			nResult = pTAS2563->write(
				pTAS2563, TAS2563_REG(nBook, nPage, nOffset),
				nData);
			if (nResult < 0)
				goto end;
			if (pBlock->mbYChkSumPresent) {
				nResult = doSingleRegCheckSum(
					pTAS2563, nBook, nPage, nOffset, nData);
				if (nResult < 0)
					goto check;
	...
	if (pBlock->mbPChkSumPresent) {
		nResult =
			pTAS2563->read(pTAS2563, TAS2563_I2CChecksum, &nValue);
...
static int tas2563_load_data(struct tas2563_priv *pTAS2563, struct TData *pData,
			     unsigned int nType)
{
	int nResult = 0;
	unsigned int nBlock;
				      ...
				      bool bLoadSame)
{
	...
	pCurrentConfiguration =
		&(pTAS2563->mpFirmware
			  ->mpConfigurations[pTAS2563->mnCurrentConfiguration]);
	pNewConfiguration =
	...
	if (pTAS2563->mbPowerUp) {
		...
		nResult = tas2563_load_coefficient(
			pTAS2563, pTAS2563->mnCurrentConfiguration,
			nConfiguration, true);
	} else {
		dev_info(
			pTAS2563->dev,
			   ...
			   unsigned int nSize)
{
	...
	if (pFirmware->mnDeviceFamily != 0) {
		dev_err(pTAS2563->dev, "deviceFamily %d, not TAS device",
	...
	if (pFirmware->mnDevice != 5) {
		dev_err(pTAS2563->dev, "device %d, not TAS2563",
			pFirmware->mnDevice);
		return -EINVAL;
	}
			 ...
			 unsigned char *pData)
{
	...
	pImageData->mpBlocks = kmalloc_array(pImageData->mnBlocks,
				 ...
				 unsigned char *pData)
{
	...
	pFirmware->mpPrograms = kmalloc_array(pFirmware->mnPrograms,
					      sizeof(struct TProgram),
					      GFP_KERNEL);
	for (nProgram = 0; nProgram < pFirmware->mnPrograms; nProgram++) {
		...
		n = fw_parse_data(pTAS2563, pFirmware, &(pProgram->mData),
				  pData);
		pData += n;
				       ...
				       unsigned char *pData)
{
	...
	pFirmware->mpConfigurations = kmalloc_array(pFirmware->mnConfigurations,
	...
	for (nConfiguration = 0; nConfiguration < pFirmware->mnConfigurations;
		...
		pData += 2;
		dev_info(pTAS2563->dev, "Fs rate: %d",
			 pConfiguration->mnFsRate);

		n = fw_parse_data(pTAS2563, pFirmware, &(pConfiguration->mData),
				  pData);
		pData += n;
	}
			      ...
			      struct TFirmware *pFirmware, unsigned char *pData)
{
	...
	for (nCalibration = 0; nCalibration < pFirmware->mnCalibrations;
		...
		n = fw_parse_data(pTAS2563, pFirmware, &(pCalibration->mData),
				  pData);
		pData += n;
	}
...
static int tas2563_set_power_state(struct tas2563_priv *pTAS2563, int state)
{
	...
	if ((pTAS2563->mpFirmware->mnPrograms == 0) ||
		...
		nResult = request_firmware_nowait(THIS_MODULE, 1, pFWName,
						  pTAS2563->dev, GFP_KERNEL,
						  pTAS2563, tas2563_fw_ready);
	...
	if (state != TAS2563_POWER_SHUTDOWN) {
		if (!pTAS2563->mbPowerUp) {
			if (!pTAS2563->mbCalibrationLoaded) {
				nResult =
					tas2563_set_calibration(pTAS2563, 0xFF);
			...
			if (pTAS2563->mbLoadConfigurationPrePowerUp) {
				...
				nResult = tas2563_load_coefficient(
					...
					pTAS2563->mnNewConfiguration, false);
				if (nResult < 0)
					goto end;
			}
		}
	...
	case TAS2563_POWER_ACTIVE:
		pTAS2563->update_bits(
			pTAS2563, TAS2563_InterruptConfiguration,
...
static const struct snd_soc_dapm_widget tas2563_dapm_widgets[] = {
	...
	SND_SOC_DAPM_OUTPUT("OUT"), SND_SOC_DAPM_SIGGEN("VMON"),
...
{
	int ret = 0;
	ret = pTAS2563->update_bits(pTAS2563, TAS2563_TDMConfigurationReg5,
				    0xff, 0x42);
	if (ret < 0)
		return ret;

	ret = pTAS2563->update_bits(pTAS2563, TAS2563_TDMConfigurationReg6,
				    0xff, 0x40);
...
static int tas2563_set_slot(struct tas2563_priv *pTAS2563, int slot_width)
{
	...
	switch (slot_width) {
	case 16:
		ret = pTAS2563->update_bits(
			pTAS2563, TAS2563_TDMConfigurationReg2,
...
static int tas2563_set_samplerate(struct tas2563_priv *pTAS2563, int samplerate)
{
	...
	case 48000:
		pTAS2563->update_bits(
			pTAS2563, TAS2563_TDMConfigurationReg0,
...
int tas2563_load_default(struct tas2563_priv *pTAS2563)
{
	...
	/*Enable TDM IRQ */
	ret = pTAS2563->update_bits(
		pTAS2563, TAS2563_InterruptMaskReg0,
				    ...
				    bool bPowerOn)
{
	...
	//prog_coefficient:
	dev_info(pTAS2563->dev,
		 "load new configuration: %s, coeff block data\n",
		 pNewConfiguration->mpName);
	nResult = tas2563_load_data(pTAS2563, &(pNewConfiguration->mData),
	...
	if (pTAS2563->mpCalFirmware->mnCalibrations) {
		nResult = tas2563_set_calibration(
			pTAS2563, pTAS2563->mnCurrentCalibration);
		if (nResult < 0)
			goto end;
	}
					    ...
					    int nReg, int *pnValue)
{
	...
	for (i = 0; i < pData->mnBlocks; i++) {
		...
		if (pBlock->mnType == blockType) {
			bFound = tas2563_get_coefficient_in_block(
				pTAS2563, pBlock, nReg, pnValue);
			if (bFound)
				break;
		}
	}
...
static bool tas2563_find_Tmax_in_configuration(
	struct tas2563_priv *pTAS2563, struct TConfiguration *pConfiguration,
	int *pnTMax)
{
	struct TData *pData;
	bool bFound = false;
	...
	bFound = tas2563_get_coefficient_in_data(pTAS2563, pData, nBlockType,
						 nReg, &nCoefficient);
	if (bFound)
		*pnTMax = nCoefficient;
	...
	if (bFound) {
		*pnValue = nCoefficient;
		dev_info(pTAS2563->dev, "%s, B[0x%x]P[0x%x]R[0x%x]=0x%x\n",
			 __func__, TAS2563_BOOK_ID(nReg), TAS2563_PAGE_ID(nReg),
			...
			int nConfig)
{
	...
	if (pTAS2563->mbPowerUp) {
		...
		nResult = tas2563_set_power_state(pTAS2563,
	...
	dev_info(pTAS2563->dev, "load program %d (%s)\n", nProgram,
		 pProgram->mpName);
	nResult = tas2563_load_data(pTAS2563, &(pProgram->mData),
				    TAS2563_BLOCK_PGM_DEV_A);
	if (nResult < 0)
		goto end;
				   ...
				   int nCalibration)
{
	...
	if (nCalibration >= pTAS2563->mpCalFirmware->mnCalibrations) {
		dev_err(pTAS2563->dev, "Calibration %d doesn't exist\n",
			nCalibration);
		nResult = 0;
		goto end;
	}
	...
	if (pProgram->mnAppMode == TAS2563_APP_TUNINGMODE) {
		if (pTAS2563->mbBypassTMax) {
			bFound = tas2563_find_Tmax_in_configuration(
				pTAS2563, pConfiguration, &nTmax);
		...
		nResult = tas2563_load_data(pTAS2563, &(pCalibration->mData),
					    TAS2563_BLOCK_CFG_COEFF_DEV_A);
		if (nResult < 0)
			goto end;
	}
	...
	if (nResult < 0) {
		tas2563_clear_firmware(pTAS2563->mpCalFirmware);
		nResult = tas2563_set_program(pTAS2563,
...
bool tas2563_get_Cali_prm_r0(struct tas2563_priv *pTAS2563, int *prm_r0)
{
	...
	pCalibration =
		&(pTAS2563->mpCalFirmware
			  ->mpCalibrations[pTAS2563->mnCurrentCalibration]);
...
static int tas2563_set_fmt(struct tas2563_priv *pTAS2563, unsigned int fmt)
{
	...
	pTAS2563->update_bits(
		pTAS2563, TAS2563_TDMConfigurationReg1,
		TAS2563_TDMConfigurationReg1_RXOFFSET51_Mask,
		(tdm_rx_start_slot
		 << TAS2563_TDMConfigurationReg1_RXOFFSET51_Shift));
				    ...
				    int slots, int slot_width)
{
	...
	dev_info(pTAS2563->dev,
		 "%s, tx_mask:%d, rx_mask:%d, slots:%d, slot_width:%d",
				   ...
				   struct snd_ctl_elem_value *pValue)
{
	...
	dev_info(pTAS2563->dev, "tas2563_calibration_get = %d\n",
...
static const char *const vboost_ctl_text[] = { "default",
					       "Device(s) AlwaysOn" };
				  ...
				  struct snd_ctl_elem_value *pValue)
{
	...
	if (vboost_state)
		pTAS2563->update_bits(
			pTAS2563, TAS2563_BoostConfiguration,
...
static const struct snd_kcontrol_new tas2563_snd_controls[] = {
	SOC_SINGLE_TLV("Amp Output Level", TAS2563_PlaybackConfigurationReg0, 0,
		       0x16, 0, tas2563_digital_tlv),
	...
	SOC_ENUM_EXT("VBoost Ctrl", vboost_ctl_enum[0], tas2563_vboost_ctl_get,
		     tas2563_vboost_ctl_put),
};
...
int tas2563_register_codec(struct tas2563_priv *pTAS2563)
{
	...
	nResult =
		snd_soc_register_codec(pTAS2563->dev, &soc_codec_driver_tas2563,
				       tas2563_dai_driver,
